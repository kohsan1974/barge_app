...
## シフトの内容物をタンク登録に縛らない — 完了
- 理由: ビルジ/廃油/重油系は処理(シフト)中に容態変化するため、タンク登録に一致しない。水は問題なし。
- 変更: シフト時のみ内容物を全アクティブItemTypeから選択可に（record/page.tsxでallContents読込→record-form.tsxのavailableContentsはisShiftOpでallContents）。サーバーはシフト時「有効なItemTypeか」だけ検証、distribute()にrequireRegistered引数を追加しシフトは登録に縛らず分配（false）。搬入/放流/出荷は従来どおり。
