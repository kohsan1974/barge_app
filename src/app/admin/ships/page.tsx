...
import { createShip, updateShip, toggleShipActive } from "@/lib/actions/ships";
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        >
          <div>
            <label className="mb-1 block text-xs text-zinc-500">本船名</label>
            <input
              name="name"
              required
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-200 bg-zinc-50 text-left text-zinc-500 dark:border-zinc-800 dark:bg-zinc-800 dark:text-zinc-400">
              <th className="px-4 py-2 font-medium">本船名</th>
            ...
            {ships.map((s) => (
              <tr key={s.id} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800">
                <td className="px-4 py-2">
                  <form action={updateShip} className="flex items-center gap-2">
