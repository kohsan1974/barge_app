...
export async function calibrateVessel(
  ...
  // 未来日・1年より前の日付を拒否（未指定なら日本時間の今日）
  const dateCheck = validateBusinessDate(businessDateRaw || todayJst());
  if (dateCheck.error || !dateCheck.date) {
    return { error: dateCheck.error ?? "業務日が正しくありません" };
  }
  const businessDate = dateCheck.date;

  // 浮動小数点の誤差を避けるため 1/100 kL 単位の整数で計算する
  const measuredCenti = toCenti(measuredRaw);
  let newBalance = 0;

  try {
    // Neonのコールドスタート対策。トランザクション開始前の失敗のみ安全に再試行される
    await withDbRetry(() =>
      prisma.$transaction(async (tx) => {
        ...
        const systemBeforeCenti = toCenti(vessel.currentBalance);
        const maxCapacityCenti = toCenti(vessel.maxCapacity);
        if (measuredCenti > maxCapacityCenti) {
          throw new Error(
            `実測値がタンクの最大容量(${fromCenti(maxCapacityCenti)}kL)を超えています。容量設定が誤っている場合は先にタンクマスタを修正してください`,
          );
        }

        const diffCenti = measuredCenti - systemBeforeCenti;
        ...
        await tx.tankTransaction.create({
          data: {
            ...
            quantity: fromCenti(diffCenti), // 差分ゼロでも「確認した」という監査記録として残す
            balanceAfter: fromCenti(measuredCenti),
            measuredValue: fromCenti(measuredCenti),
            systemValueBefore: fromCenti(systemBeforeCenti),
            reason,
          },
        });
        ...
        await tx.vessel.update({
          where: { id: vesselId },
          data: { currentBalance: fromCenti(measuredCenti) },
        });
        newBalance = fromCenti(measuredCenti);
      }),
    );
  } catch (e) {
