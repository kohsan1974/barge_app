...
- **搬入タンク（振替元）**：記録画面の「受入れタンク」の上に表示。搬入(RECEIVE)時のみ、任意で選択可（選択肢は受入れタンクと同じ部署絞り込み＋受入れタンク自身（グループの場合はグループ内タンクすべて）を除外）。`showTotalOnly`バージは受入れタンクと同様にグループ1エントリとして選択可。選択すると「タンク間振替」として扱い、同一slipIdでPROCESS（振替元・減算）とRECEIVE（振替先・加算）の行を記録し両タンクの残高を同時更新（行ロックはデッドロック回避のため対象タンクを合わせてid昇順で取得）。グループが絡む場合は振替元・振替先それぞれで複数タンクに分配され得る。内容物は振替元・振替先の両方に登録されている（グループなら和集合に含まれる）もののみ選択可。振替のreasonに`タンク間振替: 元ラベル → 先ラベル`を自動記録。新しい`TransactionType`は追加していない（PROCESS/RECEIVEの行ペアとして表現）
- タンク関連ラベルは「受入れタンク」に統一（旧「タンク」から変更）
- **タンク×部署ごとの役割フラグ**：`VesselDepartment.allowReceiving`／`allowSourcing`（両方デフォルト`true`＝リンク新規作成時の初期値）で、記録画面の「受入れタンク」「搬入タンク（振替元）」それぞれの選択肢に出すかを制御する。**バージ単位ではなく、タンクと所属部署の組ごとに個別設定**（同じタンクでも部署Aでは受入れのみ・部署Bでは搬入元のみ、といった使い分けに対応。両方trueなら両方の選択肢に出る＝同一タンク×部署が受入れ・搬入元を兼ねる運用にも対応）。所属部署のないタンクはそもそもどの部署からも選択できない（上記4番のリンク0件ルール）。`/admin/vessels`の各タンク行に、所属部署チェックボックスと「受入」「搬入」チェックボックスを並べた行を専用フィールドセットとして表示（`saveBargeSettings`が`vesselDeptReceiving_<vesselId>_<departmentId>`／`vesselDeptSourcing_<vesselId>_<departmentId>`から読み取り、`VesselDepartment`行にupsert）。`showTotalOnly`バージのグループ選択肢は、配下タンクのいずれかがその部署・役割で利用可能なら全体として選択肢に出す（実際の絞り込みはサーバー側`resolveTarget`がタンク単位で改めて行う）。クライアント側（`record-form.tsx`の`roleFor`）とサーバー側（`record-transaction.ts`の`resolveTarget`）の両方でチェックする二重防御
...
- **部署の「バージ間シフト」指定**：`Department.requiresTransfer`（デフォルト`false`）をtrueにすると、その部署の記録は種別（運搬/処理）に関わらず常に搬入タンク（移動元）・受入れタンク（移動先）の両方を必須にした振替として扱う（例: 恵比寿部署は処理部署だが実態はバージtoバージの移送のみ）。`/admin/departments`で部署ごとにチェックボックス設定。`record-form.tsx`では`isShift`がtrueの間、搬入タンクの「なし（通常の搬入）」選択肢を消し`required`にし、未選択時は先頭候補へ自動フォールバックする（受入れタンクと同じ「フォールバック」パターン）。サーバー側`record-transaction.ts`は`department.requiresTransfer`のときsourceRef必須チェックを追加し、`transactionType`が`RECEIVE`でなくても振替分岐（PROCESS+RECEIVEのペア作成）に入れるようゲートを緩和
- **管理画面の一括保存UX**：`/admin/vessels`・`/admin/departments`・`/admin/accounts`・`/admin/sites`は、行ごとの個別「保存」ボタンをやめ、**ページ内で共通の1つの右下固定ボタン**（`src/components/sticky-save-button.tsx`の`StickySaveButton`）でページ全体をまとめて保存する方式に統一した。実装は「見えない`<form id="xxx-form" action={...} />`をページに1つだけ置き、各行の入力欄・チェックボックスはDOM上の位置に関係なく`form="xxx-form"`属性でそこに紐づける」というHTML標準の仕組みを使う（`<form>`で囲むと削除・無効化用の別ミニフォームがネストして壊れるため、あえて囲まない）。サーバー側の保存アクション（`saveBargeSettings`/`saveDepartments`/`saveAccounts`/`saveSites`）は、対象idの一覧（例: `bargeIds`/`departmentIds`/`userIds`/`siteIds`）と、id付きのフィールド名（例: `bargeName_<id>`）を受け取り、全件をまとめて処理する。**`saveBargeSettings`は全バージ・全タンクをひとつの`prisma.$transaction`で処理するため、既定の5秒では容易にタイムアウトする**（タンク数×部署数ぶんの往復が発生するため）。`timeout: 30000`を明示し、かつ所属部署の`deleteMany`は各タンクごとに投げず`OR`条件でまとめて1回にすることで往復回数を抑えている。他の3画面はテーブル1行＝1回のupdateなので独立トランザクション（`$transaction`でまとめていない）のため、このタイムアウト問題は起きない
